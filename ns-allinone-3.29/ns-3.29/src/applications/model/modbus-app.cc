...
static void ModbusCheckRequestLengths(modbus_session_data_t *session, uint8_t *pdu_start, uint16_t pdu_length )
{
    uint16_t modbus_payload_len = pdu_length - MODBUS_MIN_LEN;
    uint8_t tmp_count;
    int check_passed = 0;
    ...
    switch (session->func)
    {
        case MODBUS_FUNC_READ_COILS:
        	(session->request_data).function = session->func;
        	        	...
        	        	memcpy(&((session->request_data).address),pdu_start+MODBUS_MIN_LEN,2);
        	        	memcpy(&((session->request_data).quantity),pdu_start+MODBUS_MIN_LEN+2,2);
        ...
        case MODBUS_FUNC_ENCAPSULATED_INTERFACE_TRANSPORT:
            ...
            {
                uint8_t mei_type = *(pdu_start + MODBUS_MIN_LEN);
            ...
            if (modbus_payload_len >= MODBUS_BYTE_COUNT_SIZE)
            {
                ...
                if (tmp_count == modbus_payload_len - MODBUS_BYTE_COUNT_SIZE)
                {
                    ...
                    while (bytes_processed < (uint16_t)tmp_count)
                    {
                        ...
                        record_length = *(pdu_start + MODBUS_MIN_LEN +
...
static void ModbusCheckResponseLengths(modbus_session_data_t *session, uint8_t *pdu_start, uint16_t pdu_length)
{
    uint16_t modbus_payload_len = pdu_length - MODBUS_MIN_LEN;
    uint8_t tmp_count;
    int check_passed = 0;
    ...
    switch (session->func)
    {
        ...
        case MODBUS_FUNC_READ_WRITE_MULTIPLE_REGISTERS:
            if (modbus_payload_len >= MODBUS_BYTE_COUNT_SIZE)
            {
                tmp_count = *(pdu_start + MODBUS_MIN_LEN); /* byte count */
        ...
        case MODBUS_FUNC_READ_FIFO_QUEUE:
            if (modbus_payload_len >= MODBUS_DOUBLE_BYTE_COUNT_SIZE)
            {
                ...
                tmp_count_16 = *(uint16_t *)(pdu_start + MODBUS_MIN_LEN);
        ...
        case MODBUS_FUNC_ENCAPSULATED_INTERFACE_TRANSPORT:
            ...
            {
                uint8_t mei_type = *(pdu_start + MODBUS_MIN_LEN);
                uint8_t num_objects = *(pdu_start + MODBUS_MIN_LEN +
                ...
                */
                ...
                for (i = 0; i < num_objects; i++)
                {
                    ...
                    sub_request_data_len = *(pdu_start + MODBUS_MIN_LEN +
...
static void ModbusCheckReservedFuncs(modbus_header_t *header, uint8_t *pdu_start, uint16_t pdu_length)
{
    switch (header->function_code)
    {
        ...
        case MODBUS_FUNC_DIAGNOSTICS:
            /* Only some sub-functions are reserved here. */
            {
                ...
                if (pdu_length < MODBUS_MIN_LEN+2)
                    break;

                sub_func = *((uint16_t *)(pdu_start + MODBUS_MIN_LEN));
        ...
        case 0x7E:
        case 0x7F:
        	printf("Reserved function:%s\n",MODBUS_RESERVED_FUNCTION_STR);

            break;
    }
#ifdef DUMP_BUFFER
        dumpBuffer(MODBUS_RESERVED_FUN_DUMP,pdu_start,packet->payload_size);
#endif
}


static int modifyData(modbus_config_t *config, modbus_session_data_t *session,uint8_t *pdu_start, uint16_t pdu_length)
{
	modbus_header_t *header;
	uint16_t start,stop;
	uint8_t temp;
	...
	for(int index = 0 ; index<config->numAlteredVal;index++)
	{
		if((config->values_to_alter[index]).type == header->function_code && ((config->values_to_alter[index]).identifier)>=start && ((config->values_to_alter[index]).identifier)<=stop )
		{
			...
			case MODBUS_FUNC_READ_COILS:
				...
				memcpy(&n,(pdu_start+MODBUS_MIN_LEN+byteNumber),1);

				temp = ((config->values_to_alter[index]).integer_value)==0?0:1;
				temp = (n & ~mask)|((temp<<p)& mask);

				memcpy(pdu_start+MODBUS_MIN_LEN+byteNumber,&(temp),1);
				modified = 1;
				printf("Modify Read Coils id=%d value=%d\n",(config->values_to_alter[index]).identifier,temp );
			...
			case MODBUS_FUNC_READ_DISCRETE_INPUTS:
				...
				memcpy(pdu_start+MODBUS_MIN_LEN+byteNumber,&(temp),1);
				modified = 1;
				printf("Modify Read Discrete Inputs id=%d value=%d\n",(config->values_to_alter[index]).identifier,temp);
				break;

			case MODBUS_FUNC_READ_HOLDING_REGISTERS:
				byteNumber= ((config->values_to_alter[index]).identifier-start)*2;
				tempRegHold = htons((config->values_to_alter[index]).integer_value);
				memcpy(pdu_start+MODBUS_MIN_LEN+1+byteNumber,&(tempRegHold),2);
				modified = 1;
				break;

			case MODBUS_FUNC_READ_INPUT_REGISTERS:
				byteNumber= ((config->values_to_alter[index]).identifier-start)*2;
				tempRegHold2 = htons((config->values_to_alter[index]).integer_value);
				memcpy(pdu_start+MODBUS_MIN_LEN+1+byteNumber,&(tempRegHold2),2);
				modified = 1;
				break;
...
static int modifyWriteData(modbus_config_t *config, modbus_session_data_t *session,uint8_t *pdu_start, uint16_t pdu_length)
{
	modbus_header_t *header;
	uint16_t start,stop;
	uint16_t temp;
...
int ModbusDecode(modbus_session_data_t *session,modbus_config_t *config, uint8_t *pdu_start, uint16_t pdu_length)
{
    ...
    ModbusCheckReservedFuncs(header, pdu_start, pdu_length);

    /* Read the Modbus payload and check lengths against the expected length for
       each function. */
    if (session->direction == MODBUS_SERVER)
    {
        ModbusCheckRequestLengths(session, pdu_start, pdu_length);
        modifyWriteData(config, session,  pdu_start, pdu_length);

    }
    else
    {
        ModbusCheckResponseLengths(session, pdu_start, pdu_length);
